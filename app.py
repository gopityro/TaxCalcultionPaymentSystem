...
def create_table():
    """
    Function to create the payments table in the SQLite database if it does not already exist.
    """
...
def submit():
    """
    Function to handle form submission for adding a new payment record to the database.
    """
...
def format_date(value):
    """
    Jinja2 filter to format a date string (YYYY-MM-DD) into MM/DD/YYYY format.
    """
    if value:
...
def index():
    """
    Function to render the main index page, displaying the list of payments and available due dates.
    """
    conn = sqlite3.connect(DATABASE)
    c = conn.cursor()
...
def insert_record():
    """
    Function to insert a new payment record into the database using JSON data.
    """
...
def format_date_mmddyyyy(date_str):
    """
    Function to format a date string from YYYY-MM-DD to MM/DD/YYYY.
    """
...
def summary():
    """
    Function to display a summary of payments based on the selected due date.
    """
...
def update():
    """
    Function to update an existing payment record in the database.
    """
...
def delete():
    """
    Function to delete a payment record from the database by its ID.
    """
...
if __name__ == '__main__':
    """
    Main entry point to start the Flask application in debug mode.
    """
    app.run(debug=True)